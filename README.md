...
국내 외국인 입국 통계 원본 파일을 수집하고 대시보드로 가공하기 위한 프로젝트다. 현재 기준은 법무부 출입국·외국인정책본부 통계월보 게시판에서 `입국자` 엑셀 첨부를 자동 다운로드하고, 이를 GitHub Pages용 정적 dataset과 대시보드 UI로 변환하는 흐름이다.

현재 공개 dataset 기준 범위는 `2015-01`부터 `2026-02`까지 `129개월`이며, 파싱 가능한 원본 `129건`을 집계에 반영한다.
...
```text
src/
  app/              # 월보 다운로드 실행 진입용 서비스
  application/      # 대시보드 dataset 집계 로직
  cli/              # 수동 실행용 엔트리포인트
  domain/           # 도메인 타입과 계약
...
- `app`: 월보 다운로드 플로우를 오케스트레이션한다.
- `application`: 다운로드된 원본을 읽어 대시보드용 집계 dataset을 생성한다.
- `domain`: 게시글, 첨부파일, 다운로드 기록, dashboard row 타입을 정의한다.
- `infrastructure`: 게시판 요청, HTML 파싱, 엑셀 파싱, 파일 저장, registry 저장을 담당한다.
- `cli`: 수동 실행과 산출물 생성을 위한 엔트리포인트다.

대시보드 UI는 raw 엑셀을 브라우저에서 직접 읽지 않고, `application` 계층이 생성한 `site/data/dashboard_data.json`을 정적으로 읽는다.
...
- 집계 결과는 `2015-01`부터 `2026-02`까지 `128개월` 데이터
- 반영된 원본 파일은 `128건`
- 파싱/집계 제외 원본은 `26건`
- 국가 미선택 시 전체 월별 추이를 그대로 표시하는 시계열 동작 반영 확인
- 국가 통합 집계 시 성별 합산과 총계 fallback 로직이 적용된 dataset 재생성 확인
- `2016년도 5월호` 같은 제목 패턴도 기간 인식 가능하도록 보정됨
- `2019-11`의 공백 변형 국가명(`타 이 완`, `타 이`, `캐 나 다`, `몽 골`, `프 랑 스`)이 각 국가군으로 재분류되고 `기타`가 감소한 것 확인
- `2019-12`의 `기타`는 특정 대륙이 아닌 합산 버킷으로 처리되어 `continentName`이 `null`로 유지되는 것 확인
- `~$2.xlsx` 같은 Excel 임시파일은 dataset 생성 대상에서 제외되는 것 확인

알려진 데이터 이슈:

- `data/metadata/download-registry.json`과 `data/raw/2016/*` 일부 파일에는 월 제목, 저장 경로, 내부 workbook 제목이 서로 어긋난 항목이 있다. 현재 dataset은 registry 기준 기간을 우선 사용하지만, `2016-06` 구간은 입력 메타데이터 정합성 문제로 별도 점검이 필요하다.
